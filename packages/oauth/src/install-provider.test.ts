...
// NOTE: We must use require() here (not import) because rewiremock patches require().
// ESM imports are hoisted and would execute before rewiremock.enable().
  ...
  describe('constructor()', async () => {
    ...
    it('should throw an error if missing a clientSecret', async () => {
      try {
        ...
        assert.strictEqual(e.code, ErrorCode.InstallerInitializationError);
        assert.strictEqual(e.message, 'You must provide a valid clientId and clientSecret');
      }
    });
    ...
    it('should throw an error if missing a stateSecret when using default state store', async () => {
      try {
        ...
        const e = error as InstallerInitializationError;
        assert.strictEqual(e.code, ErrorCode.InstallerInitializationError);
        assert.strictEqual(e.message, 'To use the built-in state store you must provide a State Secret');
      }
    });
  });
  ...
  describe('installer.handleInstallPath', async () => {
    it('should fail if installUrlOptions are not given', async () => {
      ...
      try {
        ...
        assert.strictEqual(e.code, ErrorCode.GenerateInstallUrlError);
      }
    });
  ...
  describe('installer.generateInstallUrl', async () => {
    it('should return a generated v2 url', async () => {
      ...
      try {
        ...
        assert.strictEqual(fakeStateStore.generateStateParam.callCount, 1);
        assert.strictEqual(fakeStateStore.verifyStateParam.callCount, 0);
        assert.strictEqual(fakeStateStore.generateStateParam.calledWith(installUrlOptions), true);

        const parsedUrl = url.parse(generatedUrl, true);
        assert.strictEqual(parsedUrl.query.state, 'fakeState');
        assert.strictEqual(parsedUrl.pathname, '/oauth/v2/authorize');
        assert.strictEqual(scopes.join(','), parsedUrl.query.scope);
        assert.strictEqual(redirectUri, parsedUrl.query.redirect_uri);
        assert.strictEqual(teamId, parsedUrl.query.team);
        assert.strictEqual(userScopes.join(','), parsedUrl.query.user_scope);
      } catch (error) {
    ...
    it('should not call generate state param when state validation is false', async () => {
      ...
      try {
        const generatedUrl = await installer.generateInstallUrl(installUrlOptions, stateVerification);
        assert.ok(generatedUrl);
        assert.strictEqual(fakeStateStore.generateStateParam.callCount, 0);
        assert.strictEqual(fakeStateStore.verifyStateParam.callCount, 0);
      } catch (error) {
        assert.fail((error as Error).message);
      }
    ...
    it('should return a generated url when passed a custom authorizationUrl', async () => {
      ...
      try {
        ...
        assert.strictEqual(parsedUrl.query.state, 'fakeState');
        assert.strictEqual(parsedUrl.pathname, '/oauth/v2/authorize');
        assert.strictEqual(parsedUrl.host, 'dev.slack.com');
        assert.strictEqual(scopes.join(','), parsedUrl.query.scope);
        assert.strictEqual(redirectUri, parsedUrl.query.redirect_uri);
        assert.strictEqual(teamId, parsedUrl.query.team);
        assert.strictEqual(userScopes.join(','), parsedUrl.query.user_scope);
    ...
    it('should fail if missing scopes', async () => {
      ...
      try {
        ...
        const e = error as GenerateInstallUrlError;
        assert.strictEqual(e.message, 'You must provide a scope parameter when calling generateInstallUrl');
        assert.strictEqual(e.code, ErrorCode.GenerateInstallUrlError);
      }
    });
  });
  ...
  describe('installer.authorize', async () => {
    it('should fail if database does not have an entry for authorize query', async () => {
      ...
      try {
        ...
        assert.strictEqual(e.code, ErrorCode.AuthorizationError);
        assert.strictEqual(
          e.message,
    ...
    it('should successfully return the Installation Object from the database', async () => {
      ...
      try {
        ...
        assert.strictEqual(authResult.userToken, fakeAuthResult.userToken);
        assert.strictEqual(authResult.botToken, fakeAuthResult.botToken);
        assert.strictEqual(authResult.botId, fakeAuthResult.botId);
        assert.strictEqual(authResult.botUserId, fakeAuthResult.botUserId);
      } catch (error) {
        assert.fail((error as Error).message);
      }
    });
  ...
  describe('installer.handleCallback', async () => {
    ...
    it('should call the failure callback with a valid installOptions due to missing code query parameter on the URL', async () => {
      ...
      const callbackOptions: CallbackOptions = {
        ...
        failure: async (error, installOptions, _req, res) => {
          ...
          assert.ok(installOptions !== undefined);
          assert.strictEqual(error.code, ErrorCode.MissingCodeError);
          res.end('failure');
        },
      };
      const installer = new InstallProvider({
    ...
    it('should call the failure callback due to missing state query parameter on the URL', async () => {
      ...
      const callbackOptions: CallbackOptions = {
        ...
        failure: async (error, installOptions, _req, res) => {
          assert.ok(installOptions !== undefined);
          assert.strictEqual(error.code, ErrorCode.MissingStateError);
          res.end('failure');
        },
      };
      const installer = new InstallProvider({
    ...
    it('should call the failure callback if an access_denied error query parameter was returned on the URL', async () => {
      ...
      const callbackOptions: CallbackOptions = {
        ...
        failure: async (error, installOptions, _req, res) => {
          assert.ok(installOptions !== undefined);
          assert.strictEqual(error.code, ErrorCode.AuthorizationError);
          res.end('failure');
        },
      };
      const installer = new InstallProvider({
    ...
    it('should call the success callback for a v2 url', async () => {
      ...
      await installer.handleCallback(req, res, successExpectedCallbackOptions);
      assert.strictEqual(sent, true);
      assert.strictEqual((fakeStateStore.verifyStateParam as sinon.SinonSpy).callCount, 1);
    });
