 ...
 * @param {Object=} opts
 * @param {?} opts.return_im - Boolean, indicates you want the full IM channel definition in the response.
 * @param {function=} optCb Optional callback, if not using promises.
 */
ImFacet.prototype.open = function open(user, opts, optCb) {
  var requiredArgs = {
    user: user
  };

  return this.makeAPICall('im.open', requiredArgs, opts, optCb);
};
