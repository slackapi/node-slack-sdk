 ...
 *   - OPENED_RTM_CONNECTION  the remote server has acked the socket and sent a `hello`, the
...
RTMClient.prototype.connect = function connect(socketUrl) {
  this.emit(CLIENT_EVENTS.OPENING_WEBSOCKET);
...
RTMClient.prototype.handleWsOpen = function handleWsOpen() {
  this.emit(CLIENT_EVENTS.OPENED_WEBSOCKET);
...
RTMClient.prototype._handleHello = function _handleHello() {
  this.connected = true;
  this.emit(CLIENT_EVENTS.OPENED_RTM_CONNECTION);
};
...
RTMClient.prototype.send = function send(message, optCb) {
  if (this.connected) {
    ...
    this.ws.send(jsonMessage, undefined, function handleWsMsgResponse(err) {
      if (!isUndefined(err)) {
