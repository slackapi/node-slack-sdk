...
RTMClient.prototype.updateMessage = function updateMessage(message, optCb) {
  ...
  if (this.connected) {
    ret = this._chat.update(message.ts, message.channel, message.text, message.opts, optCb);
  } else {
    err = new SlackRTMSendError('ws is not connected, unable to update message', message);
