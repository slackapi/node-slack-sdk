...
The RTM API isn't available for modern granular-permissions apps, and you can no longer create new legacy apps. We recommend using [Bolt for JavaScript](/bolt-js). If you have an existing RTM app, do not update its scopes as it will be updated to a granular-permissions app and stop working with the RTM API.
...
`Promise` that resolves to the data returned from the [`rtm.connect`](/reference/methods/rtm.connect) method.
...
Options passed to the `.start()` method are passed through as arguments to the [`rtm.connect`](/reference/methods/rtm.connect) method. These arguments deal with presence, which is discussed in more
...
// Attach listeners to events by type. See: /reference/events/message
...
// Attach listeners to events by message subtype. See: /reference/events/message/channel_purpose
...
[reply](/legacy/legacy-rtm-api#handling-responses). The resolved value contains information about the sent message, such as the `ts` identifier. See [error handling](#handle-errors) for details on how your app should deal with a
`Promise` rejection.
...
The Web API's [`chat.postMessage`](/reference/methods/chat.postMessage) method is capable of sending [rich
messages](/messaging/formatting-message-text#rich-layouts), more advanced layout and interactions. These rich messages are more attractive and useful for users of your app. Install and import the `@slack/web-api` package into your app, initialize the `WebClient` class, and use the `.chat.postMessage(options)` method to send a rich message. The example above can be rewritten using the following code:
...
A user's [presence](/apis/web-api/user-presence-and-status) can take one of two values: `active`
or `away`. Your app may be interested in known when a user in a workspace's presence changes. The client helps your app
set up [presence subscriptions](/apis/web-api/user-presence-and-status) to track the changes in
...
your app, and use [some of the other methods](/reference/methods) to get data.

If you're certain that you'd like to receive the snapshot, you can set the `useRtmConnect` option to `false`. This
configures the client to use the [`rtm.start`](/reference/methods/rtm.start) method (instead of
